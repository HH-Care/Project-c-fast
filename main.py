...
def reset_tracking_cache():
    ...
    # Warm up the model with a dummy frame
    dummy_frame = np.zeros((720, 1280, 3), dtype=np.uint8)
    model.predict(dummy_frame, verbose=False, half=use_half_precision)
        ...
        try:
            if tracking_enabled and selected_object_id is not None:
                if isinstance(selected_object_id, dict) and 'bbox' in selected_object_id:
                    ...
                    results = model.track(frame, persist=True, boxes=[tracking_box], half=use_half_precision)
                    ...
                    if hasattr(results[0].boxes, 'id') and results[0].boxes.id is not None:
                        ...
                        if len(track_ids) > 0:
                            ...
                            results = model(frame, half=use_half_precision)
                else:
