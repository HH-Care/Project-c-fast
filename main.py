...
@app.post("/client_webcam_frame")
async def client_webcam_frame(
    frame: UploadFile = File(...),
    tracking: bool = Form(False),
    object_id: Optional[int] = Form(None)
):
    """Process webcam frame sent from client browser and return processed frame"""
    try:
        # Read the frame data
        ...
        if img is None or img.size == 0:
            print("Error: Received empty image from client")
            # Return a simple blank image
            blank = np.zeros((480, 640, 3), dtype=np.uint8)
            ret, jpeg = cv2.imencode('.jpg', blank)
            return Response(content=jpeg.tobytes(), media_type="image/jpeg")
        
        # Process the frame with model using predict instead of track
        # This avoids the tracker_type compatibility issue
        results = model.predict(img, verbose=False)
        
        # Get result with annotations
        if tracking:
            # Just use basic detection visualization for now
            annotated_frame = results[0].plot()
            
            # Draw extra info if we're tracking a specific object
            if object_id is not None:
                # Find if this object ID exists in the current detections
                if hasattr(results[0], 'boxes') and len(results[0].boxes) > 0:
                    boxes = results[0].boxes.xywh.cpu()
                    for i, box in enumerate(boxes):
                        # Highlight this box if it's the i'th detection and we're tracking object i
                        if i == object_id:
                            x, y, w, h = box
                            x1, y1 = int(x - w/2), int(y - h/2)
                            x2, y2 = int(x + w/2), int(y + h/2)
                            # Draw a thicker green box around the tracked object
                            cv2.rectangle(annotated_frame, (x1, y1), (x2, y2), (0, 255, 0), 3)
                            # Add "TRACKING" label
                            cv2.putText(annotated_frame, "TRACKING", (x1, y1 - 10), 
                                        cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 255, 0), 2)
        else:
            # Just draw boxes and labels
            annotated_frame = results[0].plot()
        ...
        print(f"Error processing client webcam frame: {e}")
        # Return a simple error image
        error_img = np.zeros((480, 640, 3), dtype=np.uint8)
        cv2.putText(error_img, "Error processing frame", (50, 240), 
                   cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
        ret, jpeg = cv2.imencode('.jpg', error_img)
        return Response(content=jpeg.tobytes(), media_type="image/jpeg")

def process_tracking_results(frame, results, obj_id=None):
    """Process tracking results for visualization"""
    ...
    if len(results) == 0 or not hasattr(results[0], 'boxes'):
        # No detections or invalid results
        return annotated_frame
        
    # Get detection boxes and tracking IDs
    boxes = results[0].boxes.xywh.cpu()
    if hasattr(results[0].boxes, 'id') and results[0].boxes.id is not None:
    ...
    # Draw each detection and its track
    for box, track_id, cls in zip(boxes, track_ids, classes):
        # Skip if we're only tracking a specific object
        if obj_id is not None and track_id != obj_id:
            continue
        ...
        # Get current tracked position and update history
        current_position = (int(x), int(y))
        track_history[track_id].append(current_position)
        box_size_history[track_id].append((w, h))
        
        # Limit history length
        if len(track_history[track_id]) > 90:  # About 3 seconds at 30 fps
            track_history[track_id].pop(0)
            box_size_history[track_id].pop(0)
            
        # Draw bounding box
        color = (0, 255, 0) if obj_id is None or track_id == obj_id else (255, 0, 0)
...
@app.post("/stop_client_webcam")
async def stop_client_webcam():
    """Endpoint to handle client-side webcam cleanup"""
    # Reset tracking data
    reset_tracking_data()
    return {"success": True, "message": "Client webcam resources released"}
